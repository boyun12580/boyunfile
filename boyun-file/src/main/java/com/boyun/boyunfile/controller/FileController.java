...
//        log.info(userfiles.toString());
        ...
        return RestResult.success().message("创建成功！");
    }
...
//        System.out.println(deleteFileDto);
    ...
    @ResponseBody
    public RestResult<String> batchDeleteFile(@RequestBody BatchDeleteFileDTO batchDeleteFileDto,
                                              @RequestHeader("token") String token) {
    ...
    public RestResult<String> moveFile(@RequestBody MoveFileDTO moveFileDto, @RequestHeader("token") String token) {
        ...
        LambdaQueryWrapper<UserFile> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(UserFile::getFileName, moveFileDto.getFileName())
                .eq(UserFile::getFilePath, moveFileDto.getFilePath())
                .eq(UserFile::getUserId, sessionUser.getUserId());
        List<UserFile> userfiles = userfileService.list(lambdaQueryWrapper);
//        log.info(userfiles.toString());
        if (!userfiles.isEmpty()) {
